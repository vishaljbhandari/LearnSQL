...
/*
	Technique-
	Use required columns in inner select query
	User required where condition in inner select qeury
*/
-- To select selective columns use required columns in inner select query
CREATE TABLE new_customers_only_names_from_bangalore
AS (
	SELECT customer_name
	FROM customers 
	WHERE city == "Bangalore"
);  
DESC new_customers_only_names_from_bangalore;
SELECT * FROM new_customers_only_names_from_bangalore;

/* example clean up */
DROP TABLE customers;
DROP TABLE new_customers_only_names_from_bangalore;